/**
 * Script de Diagnóstico: Problema de Actualización de Precios en Cuenta Corriente (Simple)
 *
 * Versión simplificada usando BaseDiagnostic para eliminar código duplicado
 */

const { BaseDiagnostic } = require('./shared/diagnostic-base');

class PrecioCuentaCorrienteSimpleDiagnostic extends BaseDiagnostic {
    constructor() {
        super('Diagnóstico Simple - Precios en Cuenta Corriente de Mika');
    }
    ...
    async run() {
        ...
        const productosDisponibles = productos.filter(p => p.estado === 'activo' && p.stock > 0);
        ...
        const venta = await this.apiClient.post('/api/sales', ventaData);
        ...
        let deudas = await this.apiClient.get('/api/debts');
        ...
        const productoActualizado = await this.apiClient.put(`/api/products/${productoPrueba.id}`, {
        ...
        const deudaConCalculo = await this.apiClient.get(`/api/debts/${deudaVenta.id}/calcular-total`);
        ...
        const deudaActualizada = await this.apiClient.get(`/api/debts/${deudaVenta.id}`);
        ...
        try {
            const resultado = await this.apiClient.post('/api/debts/update-prices');
        ...
        const deudaFinal = await this.apiClient.get(`/api/debts/${deudaVenta.id}`);
...
const diagnostic = new PrecioCuentaCorrienteSimpleDiagnostic();
