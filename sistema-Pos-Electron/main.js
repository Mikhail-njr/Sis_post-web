...
serverApp.use('/api/products', protectWriteOperations);
serverApp.use('/api/sales', authMiddleware);
serverApp.use('/api/categories', protectWriteOperations);

// Servir archivos frontend desde carpeta ./frontend
serverApp.use(express.static(path.join(__dirname, 'frontend'), { maxAge: 0 }));
...
function initDatabaseConnection() {
    dbPath = path.join(__dirname, 'pos_database.sqlite');
...
// Rutas de activación
serverApp.get('/activate', (req, res) => {
    res.sendFile(path.join(__dirname, 'frontend/activate.html'));
});

serverApp.post('/api/activate', async (req, res) => {
...
serverApp.get('/api/license-status', async (req, res) => {
    const licenseDetails = await getLicenseDetails();
    res.json(licenseDetails);
});

// Endpoint para desactivar licencia
serverApp.post('/api/deactivate-license', authMiddleware, async (req, res) => {
    try {
...
serverApp.get('/api/can-generate-reports', async (req, res) => {
...
serverApp.get('/api/sales', async (req, res) => {
    try {
...
// Rutas para productos con búsqueda y paginación
serverApp.get('/api/products/search', async (req, res) => {
    try {
...
serverApp.get('/api/products/with-discounts', async (req, res) => {
    try {
        const products = await dbAll(`
            SELECT
                p.*,
...
serverApp.get('/api/products/:id', async (req, res) => {
    try {
...
serverApp.post('/api/products', async (req, res) => {
...
serverApp.put('/api/products/:id', async (req, res) => {
    const { codigo, nombre, descripcion, precio, stock, categoria } = req.body;
...
serverApp.post('/api/sales', async (req, res) => {
    ...
    try {
        ...
        const numeroFactura = `V${timestamp}${random}`;
        ...
        let metodoPagoFinal = paymentMethod || metodo_pago || 'No especificado';
        if (Array.isArray(pagos) && pagos.length > 0) {
            metodoPagoFinal = JSON.stringify(pagos);
        }
        ...
        try {
            ...
            const saleResult = await dbRun(
                ...
                [numeroFactura, total, metodoPagoFinal, vuelto || 0]
            );
            ...
            for (const item of items) {
                ...
                await dbRun(
                    "INSERT INTO venta_items (venta_id, producto_id, cantidad, precio_unitario, precio_original, descuento_porcentaje, subtotal) VALUES (?, ?, ?, ?, ?, ?, ?)",
                    [saleResult.id, item.id, item.cantidad, parseFloat(item.precio), parseFloat(precioOriginal), descuentoPorcentaje, subtotal]
                );
            ...
            logOperation(
                'VENTA',
                `Venta realizada: ${numeroFactura} - Total: ${formatCurrency(total)}`,
                'Sistema',
                'ventas',
                saleResult.id,
                null,
...
serverApp.get('/api/cierres', async (req, res) => {
    try {
        const cierres = await dbAll("SELECT * FROM cierres_caja ORDER BY fecha DESC");
        res.json(cierres);
    } catch (error) {
        res.status(500).json({ error: error.message });
    }
...
serverApp.post('/api/close-register-preview', async (req, res) => {
    try {
        const { fecha, dineroInicial } = req.body;

        if (!dineroInicial || isNaN(parseFloat(dineroInicial))) {
            return res.status(400).json({ error: 'Dinero inicial requerido' });
        }

        const fechaFiltro = fecha ? new Date(fecha).toISOString().split('T')[0] : new Date().toISOString().split('T')[0];

        // Obtener ventas del día
        const ventas = await dbAll(
            "SELECT total, metodo_pago FROM ventas WHERE DATE(created_at) = DATE(?)",
            [fechaFiltro]
        );

        // Calcular total de ventas
        const total = ventas.reduce((sum, venta) => sum + parseFloat(venta.total || 0), 0);

        // Calcular total esperado
        const totalEsperado = parseFloat(dineroInicial) + total;

        // Calcular diferencia (por ahora 0, se calculará al cerrar)
        const diferencia = 0;
        ...
        ventas.forEach(venta => {
            try {
                const pagos = JSON.parse(venta.metodo_pago);
                if (Array.isArray(pagos)) {
                    pagos.forEach(pago => {
                        if (pago.metodo && pago.monto) {
                            paymentTotals[pago.metodo] = (paymentTotals[pago.metodo] || 0) + parseFloat(pago.monto);
                        }
                    });
                } else {
                    // Método simple
                    paymentTotals[venta.metodo_pago] = (paymentTotals[venta.metodo_pago] || 0) + parseFloat(venta.total);
                }
            } catch (e) {
                // Método simple
                paymentTotals[venta.metodo_pago] = (paymentTotals[venta.metodo_pago] || 0) + parseFloat(venta.total);
        ...
        res.json({
            ...
            dinero_inicial: parseFloat(dineroInicial),
            total: total,
        ...
        console.error('Error calculando preview de cierre:', error);
        res.status(500).json({ error: 'Error interno del servidor: ' + error.message });
    }
});

serverApp.post('/api/close-register-confirm', async (req, res) => {
    try {
        const { fecha, dinero_inicial, total, total_esperado, diferencia, cantidad_ventas } = req.body;

        // Insertar cierre de caja
        const result = await dbRun(
            "INSERT INTO cierres_caja (fecha, dinero_inicial, total_ventas, total_esperado, diferencia, cantidad_ventas) VALUES (?, ?, ?, ?, ?, ?)",
        ...
        logOperation(
            'CIERRE_CAJA',
            `Cierre de caja realizado - Total esperado: ${formatCurrency(total_esperado)}`,
            'Sistema',
            'cierres_caja',
            result.id,
            null,
...
serverApp.get('/api/suppliers', async (req, res) => {
    try {
...
serverApp.get('/api/suppliers/:id', async (req, res) => {
    try {
...
serverApp.post('/api/suppliers', async (req, res) => {
...
serverApp.put('/api/suppliers/:id', async (req, res) => {
    const { nombre_proveedor, nombre_contacto, telefono, email, productos_servicios, condiciones_pago, estatus, notas } = req.body;
...
serverApp.delete('/api/suppliers/:id', async (req, res) => {
    try {
        const supplierId = req.params.id;
...
serverApp.get('/api/promotions', async (req, res) => {
    try {
...
serverApp.get('/api/promotions/:id', async (req, res) => {
    try {
...
serverApp.post('/api/promotions', async (req, res) => {
...
serverApp.delete('/api/promotions/:id', async (req, res) => {
    try {
        const promotionId = req.params.id;
...
serverApp.delete('/api/sales/:id', async (req, res) => {
    try {
...
serverApp.post('/api/clean-duplicate-promotions', authMiddleware, async (req, res) => {
    try {
...
// Ruta de diagnóstico
serverApp.get('/api/diagnostic', async (req, res) => {
    try {
...
serverApp.post('/api/set-time', authMiddleware, (req, res) => {
...
function createWindow() {
    ...
    mainWindow = new BrowserWindow({
        ...
        webPreferences: {
            ...
            preload: path.join(__dirname, 'preload.js') // Opcional: para comunicación segura
        },
        icon: path.join(__dirname, 'assets/icon.png'), // Opcional: icono de la app
        show: false, // No mostrar hasta que esté listo
        title: 'Sistema POS Electron'
    });
    ...
    mainWindow.once('ready-to-show', () => {
        mainWindow.show();
        console.log('🖥️ Ventana principal de Electron lista');
    });

    // Evento cuando se cierra la ventana
    mainWindow.on('closed', () => {
        mainWindow = null;
        // Cerrar servidor cuando se cierra la ventana
        if (serverProcess) {
            serverProcess.close(() => {
                console.log('🛑 Servidor Express cerrado desde ventana');
            });
        }
    });
...
// Iniciar servidor Express con manejo de errores
serverProcess = serverApp.listen(PORT, () => {
    console.log(`✅ Servidor Express iniciado en puerto ${PORT}`);
}).on('error', (err) => {
    if (err.code === 'EADDRINUSE') {
        console.error(`❌ Puerto ${PORT} ya está en uso. Intente con otro puerto o libere el puerto actual.`);
        process.exit(1);
    } else {
...
// Crear ventana principal cuando la app esté lista (solo si app está disponible)
if (app && typeof app.whenReady === 'function') {
    app.whenReady().then(createWindow);
} else {
    // Fallback para versiones anteriores de Electron o si app no está disponible
    setTimeout(() => {
        createWindow();
    }, 1000);
}

// Event listeners para Electron (solo si app está disponible)
if (app && typeof app.on === 'function') {
    app.on('activate', () => {
        if (BrowserWindow && typeof BrowserWindow.getAllWindows === 'function' && BrowserWindow.getAllWindows().length === 0) {
            createWindow();
        }
    });
    ...
    // Fallback para cuando app no está disponible
    console.log('⚠️ App de Electron no disponible, ejecutando en modo servidor solo');
}

// IPC handlers para comunicación con renderer process (opcional)
