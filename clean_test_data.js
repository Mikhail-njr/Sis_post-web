...
console.log('🧹 Limpiando datos de prueba de cuenta corriente...\n');
...
db.serialize(() => {
    ...
    db.run("BEGIN TRANSACTION", (err) => {
        ...
        try {
            ...
            // 4. Eliminar clientes de prueba (excepto los originales)
            db.run("DELETE FROM clientes WHERE id NOT IN (1, 2, 3, 4, 5, 6)", (err) => {
                if (err) {
                    console.error('❌ Error eliminando clientes de prueba:', err.message);
                    return;
                }
